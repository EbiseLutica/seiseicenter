/* 生成センター v1.8 (2017/02/26) 生成ボタンのセリフのランダム化 */
/* 生成センター v1.9 (2017/02/26) new Array()を[]に変更 */
/* 生成センター v2.0 (2017/02/28) フォームの各項目の値をCookieに保存 */
/* 生成センター v2.1 (2017/04/19) CookieからlocalStorageに移行 */
/* 生成センター v2.2 (2017/04/27) 6月2日の準備 */
/* 生成センター v2.2.1 (2017/05/18) 七月の準備 */
/* 生成センター v2.2.2 (2017/06/02) 月判定の修正 */
/* 生成センター v2.3 (2017/09/21) 設定保存のcheckbox対応 (仮) */
/* 生成センター v2.4 (2017/09/21) 生成コア部の重複防止ルーチンのバグ修正 (少し遅くなります) */
/* 生成センター v2.5 (2018/09/18) 生成コア部の重複防止ルーチンの調整 (SFC/GB向け) */
/* 生成センター v2.6 (2018/09/18) SFCの再生成が機能していなかったのを修正 */
...
function load_config() {
	if (localStorage) {
		for (var i = 0; i < localStorage.length; i++) {
			...
			if (pair.length >= 2) {
				var target = document[pair[0]][pair[1]];

				if (target) {
					if (target.length) {
						for (var j = 0; j < target.length; j++) {
							if (target[j].type == "checkbox") {
								if (pair.length >= 3) {
									if (target[j].value == pair[2]) {
										if (value == "true") {
											target[j].checked = "checked";
										} else {
											delete target[j].checked;
										}
									}
								}
							} else if (target[j].type == "radio") {
								if (target[j].value == value) {
									target[j].checked = "checked";
								} else {
									delete target[j].checked;
								}
							} else if (target[j].type != "button") {
								target[j].value = value;
							}
						}
					} else if (target.type == "checkbox") {
						if (pair.length >= 3) {
							if (target.value == pair[2]) {
								if (value == "true") {
...
function save_config(form) {
	if (localStorage) {
		...
		for (var i = 0; i < elems.length; i++) {
			...
			if (target_elem.type == "checkbox") {
				localStorage.setItem(form.name + "." + target_elem.name + "." + target_elem.value, target_elem.checked ? "true" : "false");
			} else if (target_elem.type == "radio") {
				if (target_elem.checked) {
					localStorage.setItem(form.name + "." + target_elem.name, target_elem.value);
				}
			} else if (target_elem.type != "button") {
				localStorage.setItem(form.name + "." + target_elem.name, target_elem.value);
			}
		}
	}
...
function randomize_button() {
	...
	var seiText = textList[Math.floor(Math.random() * textList.length)];

	// 日付イベント
	var date = new Date();

	if (date) {
		if (date.getMonth() == 5 && date.getDate() == 2) {
			seiText = "6月2日に　体が動かない！";
		} else if (date.getMonth() == 6 && date.getDate() == 1) {
			seiText = "七月";
		}
	}

	var seiButtons = document.getElementsByClassName("seisei-button");

	for (var i = 0; i < seiButtons.length; i++) {
		seiButtons[i].value = seiText;
	}
}
...
function generate_nes(regenerate) {
	...
	save_config(document.urajijo_nes);
}
...
function generate_sg(regenerate) {
	...
	save_config(document.urajijo_sg);
}
...
function generate_gb(regenerate) {
	...
	if (regenerate) { // 再生成: アドレスを変えずにコードを再生成
		...
		var lram_num = parseInt(document.urajijo_gb.lram_num.value);
		...
		if (ram_num > 0x1000) {
			errorOut.warn("RAM前半領域コードの数が" + 0x1000 + "個に制限されます。");
			ram_num = 0x1000;
		}

		if (lram_num > 0x1000) {
			errorOut.warn("白黒RAM後半領域コードの数が" + 0x1000 + "個に制限されます。");
			lram_num = 0x1000;
		}
		...
		codes = codes.concat(generator_core(0x01D000, 0x01DFFF, 0x00, 0xFF, lram_num, 0x10000, 1));
	...
	save_config(document.urajijo_gb);
}
...
function generate_snes(regenerate) {
	...
	save_config(document.urajijo_snes);
}
...
function generate_md(regenerate) {
	...
	save_config(document.urajijo_md);
}
...
function generate_gba(regenerate) {
	...
	save_config(document.urajijo_gba);
}
...
function generate_usa(regenerate) {
	...
	save_config(document.urajijo_usa);
}
...
function generate_hex() {
	...
	save_config(document.urajijo_hex);
}
...
function generate_bench() {
	...
	save_config(document.urajijo_bench);
}
